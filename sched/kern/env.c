...
EnvStatistics sched_stats;  // estadisticas importantes del scheduler

void
initialize_stats()
{
	sched_stats.total_env_runs = 0;
	sched_stats.amount_priority_upgrade = 0;
	sched_stats.calls_to_sched = 0;
}
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	env->env_type = type;
	env->priority =
	        0;  // uso 0 como la prioridad mas baja, todos los procesos se crean con prioridad 0
}
...
env_run(struct Env *e)
{
	...
	curenv = (curenv == NULL) ? e : curenv;
	// another environment is running -> context switch
	if (curenv != e) {
		if (curenv->env_status == ENV_RUNNING) {
			curenv->env_status = ENV_RUNNABLE;  // 1.
		}
		// it can be ENV_DYING, ENV_NOT_RUNNABLE
	}
	curenv = e;                        // 2.
	curenv->env_status = ENV_RUNNING;  // 3.
	curenv->env_runs++;                // 4.
	env_load_pgdir(curenv);            // 5.
