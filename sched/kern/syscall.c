...
static int
sys_reduce_env_priority(envid_t envid)
{
	int r;
	struct Env *env;
	...
	if ((r = envid2env(envid, &env, 1)) < 0)
		return r;
	if (env->priority < WORST_PRIORITY) {
		env->priority++;
		return 0;
	}
	return -1;
...
sys_exofork(void)
{
	...
	// Se debe incluir soporte para prioridades en las syscalls relevantes.
	// Por ejemplo, cuando un proceso llama a fork,
	// se deberá configurar acordemente (y siguiendo algún criterio) las
	// prioridades del proceso hijo. misma prioridad: es lo mas simple y al
	// ser procesos relacionados nos parece que tiene sentido que tengan la
	// misma prioridad
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_reduce_env_priority:
		return sys_reduce_env_priority(a1);
	case SYS_yield:
		sys_yield();  // No return
	default:
		return -E_INVAL;
